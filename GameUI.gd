...
func show_buy_prompt(player: Player, cell: Cell):
	...
	if cell is PropertyCell:
		...
		buy_panel.get_node("VBox/BuyTitle").text = "🏠 Mua " + cell.data.cell_name + "?"
		var rent_text = "Thuê: $" + str(prop_data.base_rent if prop_data else 0) if cell is PropertyCell else ""
		buy_panel.get_node("VBox/BuyInfo").text = "Giá: $" + str(prop_data.buy_price if prop_data else 0) + "  |  " + rent_text + "\nSố dư: $" + str(player.state.balance)
...
func show_build_prompt(player: Player, cell: Cell):
	...
	build_panel.get_node("VBox/BuildInfo").text = cell.data.cell_name + " | Chi phí: $" + str(cell.get_build_cost() if cell is PropertyCell else 0) + "\nHiện có: " + str(cell.house_count) + "/5 | Số dư: $" + str(player.state.balance)
...
func _on_build_yes():
	build_panel.visible = false
	set_roll_enabled(true)
	if _current_build_cell and _current_build_cell is PropertyCell and _current_build_cell.house_count < 5:
		_current_build_cell.build_house()
		show_message("Đã xây nhà trên " + _current_build_cell.data.cell_name)
...
func auto_mortgage_for_test(player: Player, amount_needed: int):
	...
	for cell in player.properties:
		if not cell.is_mortgaged and player.state.balance < target:
			...
			if amt > 0:
				print("> Thế chấp: ", cell.data.cell_name, " -> $", amt)
