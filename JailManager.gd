...
# =========================
# NGÀY 2 – RA TÙ (Luồng rẽ nhánh 7.2.x)
# =========================

## 7.1.5 → 7.1.6  Xử lý toàn bộ lượt của người đang ở tù sau khi tung xúc xắc.
...
## 7.2.3 → 7.2.4  Dùng thẻ Ra Tù miễn phí.
func _try_escape_with_card(player: Player, dice_result: DiceResult) -> bool:
	print("[JailManager] ", player.name, " dùng thẻ Ra Tù (BR-28 nhánh 7.2.3)")

	# 7.2.4 – Thu hồi 1 thẻ từ kho đồ
	player.state.special_cards -= 1

	# 7.1.6 – Gỡ trạng thái tù
	...
	# Cho phép di chuyển theo điểm xúc xắc (7.1.6)
	await _gc.move_player(player, dice_result.total())
	await _gc.handle_landed_cell(player, player.state.position)
	return true


## 7.2.1 → 7.2.2  Đổ double → ra tù, di chuyển theo tổng xúc xắc.
func _try_escape_with_double(player: Player, dice_result: DiceResult) -> bool:
	print("[JailManager] ", player.name, " đổ Double – thoát tù! (BR-28 nhánh 7.2.1)")

	# 7.1.6 – Gỡ trạng thái tù, đặt lại bộ đếm
	_release_from_jail(player)

	_gc.ui.show_message(player.name + " đổ Double – Thoát Tù!")
	_gc.ui.add_history(player.name + " thoát tù bằng Double", Color("#1B5E20"))

	# Di chuyển bình thường theo tổng xúc xắc (7.1.6)
	await _gc.move_player(player, dice_result.total())
	await _gc.handle_landed_cell(player, player.state.position)
	return true


## 7.2.5 → 7.2.6  Cưỡng chế nộp phạt $50 sau khi hết 3 lượt.
func _force_pay_fine(player: Player, dice_result: DiceResult) -> bool:
	...
	# 7.2.6 – Kiểm tra số dư trước khi nộp
	var paid: bool = await _pay_fine(player, dice_result)
	return paid


## 7.2.5 (chủ động)  Người chơi tự nguyện nộp phạt $50 để ra tù.
func pay_fine_voluntarily(player: Player, dice_result: DiceResult) -> bool:
	print("[JailManager] ", player.name, " chủ động nộp phạt $", JAIL_FINE, " (BR-28 nhánh 7.2.5)")
...
func _pay_fine(player: Player, dice_result: DiceResult) -> bool:
	...
	if player.state.balance >= JAIL_FINE:
		...
		# 7.1.6 – Di chuyển theo xúc xắc
		await _gc.move_player(player, dice_result.total())
		await _gc.handle_landed_cell(player, player.state.position)
		return true

	# Không đủ tiền mặt → luồng ngoại lệ 7.3.x (xử lý ở Ngày 3)
	return await _handle_fine_broke(player, dice_result)


## 7.1.6 helper: xóa trạng thái tù, reset bộ đếm.
func _release_from_jail(player: Player) -> void:
	player.state.set_in_jail(false)   # set_in_jail(false) cũng reset jail_turns = 0
	print("[JailManager] ", player.name, " đã Ra Tù (7.1.6)")


# =========================
# NGÀY 3 – LUỒNG NGOẠI LỆ (7.3.x) & TÍCH HỢP GAME CONTROLLER
# =========================

## 7.3.1 → 7.3.3  Xử lý khi người chơi không đủ $50 tiền mặt để nộp phạt.
func _handle_fine_broke(player: Player, dice_result: DiceResult) -> bool:
	print("[JailManager] ", player.name, " thiếu tiền mặt cho phạt tù (7.3.1)")

	# 7.3.1 – Tính tổng khả năng tài chính (tiền mặt + giá trị thế chấp)
	...
	# 7.3.2 – Đủ khả năng tài chính (nhờ thế chấp)
	if total_capacity >= JAIL_FINE:
		print("[JailManager] 7.3.2 – Yêu cầu thế chấp để gom đủ $", JAIL_FINE)
		_gc.ui.show_message(
		...
		if player.state.balance >= JAIL_FINE:
			...
			print("[JailManager] 7.3.3 – Vẫn thiếu tiền sau thế chấp → phá sản")
			_declare_jail_bankruptcy(player)
			return false

	# 7.3.3 – Hoàn toàn không đủ tài chính → phá sản
	print("[JailManager] 7.3.3 – Tổng tài sản < $", JAIL_FINE, " → kích hoạt phá sản (BR-30, BR-31)")
	_declare_jail_bankruptcy(player)
	return false


## 7.3.3  Tuyên bố phá sản: giải phóng tài sản, loại khỏi ván (BR-30, BR-31).
...
func handle_jail_turn(player: Player, dice_result: DiceResult) -> bool:
	# 7.1.4 – Tăng bộ đếm lượt tù (begin_jail_turn được gọi ở start_turn)
